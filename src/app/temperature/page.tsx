...
export default async function TemperaturePage({ searchParams }: { searchParams: Promise<{ group?: string }> }) {
  ...
  type MeasurementRecord = { performed_at: string | null; slot_code: string | null; measurement_details: Array<{ temperature_c: number | null; humidity_pct: number | null }> | { temperature_c: number | null; humidity_pct: number | null } | null };
  const chartPoints = (data.records as unknown as MeasurementRecord[]).flatMap((record) => { const detail = Array.isArray(record.measurement_details) ? record.measurement_details[0] : record.measurement_details; return detail?.temperature_c == null ? [] : [{ label: record.performed_at ? new Date(record.performed_at).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit", timeZone: "Asia/Ho_Chi_Minh" }) : record.slot_code ?? "—", value: Number(detail.temperature_c) }]; });
  return <AppShell headerTitle="Nhiệt độ & Độ ẩm" headerSubtitle="BM.01 · BM.02 · BM.03" unreadCount={unread}>
    <div className="space-y-6">
      ...
      <OperationalChart title="Xu hướng nhiệt độ thực tế" unit="°C" points={chartPoints} description="Không nội suy điểm đo còn thiếu" />
      <section><div><h2 className="text-xl font-black">Phiếu ca hiện tại</h2><p className="mt-1 text-xs font-semibold text-slate-500">Không nội suy dữ liệu chưa đo</p></div>{filtered.length ? <div className="mt-4 grid gap-3 lg:grid-cols-2">{filtered.map((row) => { const completed = row.status !== "PENDING"; const code = row.register_periods.form_template_versions.form_templates.code; const label = row.register_periods.locations?.name ?? row.register_periods.assets?.source_name ?? row.register_periods.form_template_versions.form_templates.name; return <article key={row.id} className="rounded-3xl border border-cyan-100 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-bold text-teal-700">{code} · {row.slot_code}</p><h3 className="mt-1 font-black text-slate-950">{label}</h3>{row.register_periods.assets?.storage_purpose ? <p className="text-sm text-slate-500">{row.register_periods.assets.storage_purpose}</p> : null}</div><span className={`inline-flex shrink-0 items-center gap-1 whitespace-nowrap rounded-full px-3 py-1 text-xs font-bold ${completed ? "bg-emerald-50 text-emerald-800" : "bg-amber-50 text-amber-800"}`}>{completed ? <CheckCircle2 className="size-3.5" /> : <Clock3 className="size-3.5" />}{completed ? (row.status === "N_A" ? "Không áp dụng" : "Đã ghi") : "Chưa đo"}</span></div><Link href={row.fulfilled_by_record_id ? `/records/${row.fulfilled_by_record_id}` : `/entry/${row.id}`} className="mt-4 inline-flex min-h-11 items-center rounded-2xl bg-teal-800 px-4 font-bold text-white">{completed ? "Xem chi tiết" : "Ghi số đo"}</Link></article>; })}</div> : <div className="mt-4"><EmptyState title="Không có điểm đo" description="Không có nghĩa vụ phù hợp với bộ lọc hôm nay." icon={<CircleAlert />} /></div>}</section>
    </div>
  </AppShell>;
}
