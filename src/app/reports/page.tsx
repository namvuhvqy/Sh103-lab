...
export default async function ReportsPage() {
  ...
  return <AppShell headerTitle="Báo cáo & Thống kê" headerSubtitle="Dữ liệu thật theo kỳ" unreadCount={unread}>
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3"><div className="clinical-card p-3 sm:p-4"><p className="text-[11px] font-bold text-slate-500">Đã phê duyệt</p><p className="mt-1 text-xl font-black text-emerald-800 sm:text-2xl">{approved.length}</p></div><div className="clinical-card p-3 sm:p-4"><p className="text-[11px] font-bold text-slate-500">Chờ duyệt</p><p className="mt-1 text-xl font-black text-amber-800 sm:text-2xl">{ready.length}</p></div><div className="clinical-card p-3 sm:p-4"><p className="text-[11px] font-bold text-slate-500">Đang mở</p><p className="mt-1 text-xl font-black text-sky-800 sm:text-2xl">{open.length}</p></div></div>
      <StatusDistribution title="Phân bố trạng thái kỳ" items={[{label:"Đã phê duyệt",value:approved.length,tone:"green"},{label:"Chờ duyệt",value:ready.length,tone:"amber"},{label:"Đang mở",value:open.length,tone:"teal"}]} />
      <div className="flex flex-wrap gap-3"><Link href="/reports/export" className="inline-flex min-h-11 items-center gap-2 rounded-2xl bg-teal-800 px-5 font-bold text-white"><FileSpreadsheet className="size-4"/>Xuất biểu mẫu</Link><Link href="/dashboard" className="inline-flex min-h-11 items-center rounded-2xl border border-teal-200 bg-white px-5 font-bold text-teal-800">Mở Dashboard</Link><Link href="/periods" className="inline-flex min-h-11 items-center rounded-2xl border border-teal-200 bg-white px-5 font-bold text-teal-800">Xem tất cả kỳ</Link></div>
      <section><h2 className="text-xl font-black">Kỳ đã phê duyệt có thể xuất</h2>{approved.length ? <div className="mt-4 space-y-3">{approved.map((period) => <article key={period.id} className="rounded-3xl border border-cyan-100 bg-white p-5 shadow-sm"><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center"><div><p className="text-xs font-bold text-teal-700">{period.form_template_versions.form_templates.code} · {period.form_template_versions.version_label}</p><h3 className="mt-1 font-black text-slate-950">{period.form_template_versions.form_templates.name}</h3><p className="mt-1 text-sm text-slate-500">{period.locations?.name ?? period.assets?.source_name ?? "Toàn khoa"} · {period.period_label ?? `${period.period_start} – ${period.period_end}`}</p><p className="mt-1 text-xs font-semibold text-emerald-700">Đã phê duyệt · {period.approved_at ? new Date(period.approved_at).toLocaleString("vi-VN") : ""}</p></div><div className="flex flex-wrap gap-2"><Link href={`/periods/${period.id}`} className="inline-flex min-h-11 items-center rounded-2xl border border-slate-200 px-4 font-bold text-slate-700">Xem trước</Link><a href={`/api/reports/${period.id}/pdf`} className="inline-flex min-h-11 items-center gap-2 rounded-2xl bg-teal-800 px-4 font-bold text-white"><Download className="size-4" />PDF</a><a href={`/api/reports/${period.id}/xlsx`} className="inline-flex min-h-11 items-center gap-2 rounded-2xl bg-sky-800 px-4 font-bold text-white"><FileSpreadsheet className="size-4" />Excel (.xlsx)</a><a href={`/api/reports/${period.id}/csv`} className="inline-flex min-h-11 items-center rounded-2xl border border-sky-200 px-4 font-bold text-sky-800">CSV</a></div></div></article>)}</div> : <div className="mt-4"><EmptyState title="Chưa có kỳ được phê duyệt" description="Kỳ đang mở hoặc chờ duyệt không được dùng làm báo cáo chính thức." /></div>}</section>
    </div>
  </AppShell>;
}
