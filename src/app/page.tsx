...
export default async function Home() {
  ...
  const total = shiftData?.assets.length ?? 0;
  const progress = shiftProgress(completed, total);
  return <AppShell headerTitle="SH103-Lab" headerSubtitle="Khoa Sinh hóa — Bệnh viện Quân y 103"><div className="space-y-8">
    <section className="overflow-hidden rounded-3xl bg-slate-950 p-6 text-white shadow-xl"><p className="text-sm font-bold uppercase tracking-wider text-blue-300">Ca trực hiện tại</p><div className="mt-2 flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-bold">{shiftData?.shift.label}</h1><p className="mt-1 text-slate-300">{progress.completed}/{progress.total} máy đã ghi nhận</p></div><Link href="/bm06" className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-blue-500 px-5 font-bold text-white">Nhập nhanh ca trực <ArrowRight className="h-4 w-4"/></Link></div><div className="mt-5 h-2 overflow-hidden rounded-full bg-slate-700" aria-label={`Tiến độ ca ${progress.percent}%`}><div className="h-full rounded-full bg-blue-400" style={{width:`${progress.percent}%`}}/></div></section>
