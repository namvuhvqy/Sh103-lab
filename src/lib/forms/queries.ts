...
export interface PeriodView { id: string; period_label: string | null; status: string; period_start: string; period_end: string; lock_version: number; approved_by: string | null; approved_at: string | null; returned_reason: string | null; locations: { code: string; name: string } | null; assets: { source_name: string } | null; form_template_versions: { form_templates: { code: string; name: string } }; }
export interface RecordView { id: string; period_id: string; record_type: string; business_date: string; slot_code: string | null; record_state: string; revision_no: number; revision_of_record_id: string | null; is_effective: boolean; is_na: boolean; note: string | null; register_periods?: { status: string }; measurement_details: unknown; decontamination_details: unknown; maintenance_details: unknown; equipment_shift_details: unknown; equipment_shift_statuses: Array<{ asset_display_order_snapshot: number; status_code: string; asset_label_snapshot: string }>; }
...
export async function getPeriods(): Promise<PeriodView[]> {
  ...
  const { data } = await supabase.from("register_periods").select("id,period_label,status,period_start,period_end,lock_version,approved_by,approved_at,returned_reason,locations(code,name),assets(source_name),form_template_versions(form_templates(code,name))").order("period_start", { ascending: false });
  return (data ?? []) as unknown as PeriodView[];
}
export async function getPeriod(id: string): Promise<{ period: PeriodView; approvedByName: string | null; occurrences: Array<{ id: string; business_date: string | null; slot_code: string | null; status: string; window_start: string; window_end: string; fulfilled_by_record_id: string | null }>; records: RecordView[] } | null> {
  const supabase = await createClient();
  const [{ data: period }, { data: occurrences }, { data: records }] = await Promise.all([
    supabase.from("register_periods").select("id,period_label,status,period_start,period_end,lock_version,approved_by,approved_at,returned_reason,locations(code,name),assets(source_name),form_template_versions(form_templates(code,name))").eq("id", id).single(),
    ...
    supabase.from("records").select("id,period_id,record_type,business_date,slot_code,record_state,revision_no,revision_of_record_id,is_effective,is_na,note,register_periods(status),measurement_details(temperature_c,humidity_pct,temperature_abnormal,humidity_abnormal),decontamination_details(daily_done,weekly_done,spill_event_done),maintenance_details(cadence,result),equipment_shift_details(usage_value,usage_unit,equipment_shift_statuses(asset_display_order_snapshot,status_code,asset_label_snapshot))").eq("period_id", id).eq("is_effective", true).order("business_date"),
  ]);
  const normalizedRecords = ((records ?? []) as unknown as RawRecordView[]).map(normalizeRecord);
  if (!period) return null;
  const typedPeriod = period as unknown as PeriodView;
  let approvedByName: string | null = null;
  if (typedPeriod.approved_by) {
    const { data: approver } = await supabase.from("profiles").select("full_name").eq("user_id", typedPeriod.approved_by).maybeSingle();
    approvedByName = approver?.full_name ?? null;
  }
  return { period: typedPeriod, approvedByName, occurrences: occurrences ?? [], records: normalizedRecords };
}
export async function getRecord(id: string): Promise<RecordView | null> {
  const supabase = await createClient();
  const { data, error } = await supabase.from("records").select("id,period_id,record_type,business_date,slot_code,record_state,revision_no,revision_of_record_id,is_effective,is_na,note,register_periods(status),measurement_details(temperature_c,humidity_pct,temperature_abnormal,humidity_abnormal),decontamination_details(daily_done,weekly_done,spill_event_done),maintenance_details(cadence,result),equipment_shift_details(usage_value,usage_unit,equipment_shift_statuses(asset_display_order_snapshot,status_code,asset_label_snapshot))").eq("id", id).maybeSingle();
...
export async function getRecordRevisionChain(record: Pick<RecordView, "id" | "revision_of_record_id">) {
  const supabase = await createClient();
  const rootId = record.revision_of_record_id ?? record.id;
  const { data, error } = await supabase.from("records")
    .select("id,revision_no,is_effective,record_state,entered_at")
    .or(`id.eq.${rootId},revision_of_record_id.eq.${rootId}`)
    .order("revision_no");
  if (error) throw new Error(`Không tải được lịch sử đính chính: ${error.message}`);
  return data ?? [];
}
