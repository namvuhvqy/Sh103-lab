...
const [locations, equipment, versions, bm06Assets, profiles, authResponse] = await Promise.all([
  ...
  get("/rest/v1/profiles?select=user_id,active"),
...
if (!profiles.some(profile => profile.active)) failures.push("no active application profile exists");
if (failures.length) {
  console.error("REMOTE AUDIT FAILED:");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}
console.log(JSON.stringify({ ok: true, locations: locations.length, equipment: equipment.length, areaCounts, publishedVersions: versions.length, bm06SnapshotAssets: bm06Assets.length, authUsers: users.length, activeProfiles: profiles.filter(profile => profile.active).length }));
